...
(defn- tv-ensure-editable [root node]
  (if (identical? (.-edit node) (.-edit root))
    node
    (VectorNode. (.-edit root) (aclone (.-arr node)))))
...
(defn- tv-push-tail [tv level parent tail-node]
  (let [ret    (tv-ensure-editable (.-root tv) parent)
...
(defn- tv-pop-tail [tv level node]
  (let [node   (tv-ensure-editable (.-root tv) node)
...
(deftype TransientVector [^:mutable cnt
  ...
  (assoc! [tcoll k v] (.assoc-n! tcoll k v))
    ...
    (if (.-edit root)
      (cond
        ...
        (if (<= (tail-off tcoll) n)
          ...
          (let [new-root
                ((fn go [level node]
                   (let [node (tv-ensure-editable root node)]
                     (if (zero? level)
      ...
      (cond
        ...
        (let [new-tail (editable-array-for tcoll (- cnt 2))
          ...
          (if (and (< 5 shift) (coercive-= (pv-aget new-root 1) nil))
            (let [new-root  (tv-ensure-editable root (pv-aget new-root 0))]
              (set! root  new-root)
              (set! shift (- shift 5))
              (set! cnt   (dec cnt))
              (set! tail  new-tail)
