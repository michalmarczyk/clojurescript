...
(set! cljs.core.ObjMap/HASHMAP_THRESHOLD 512)
...
(deftype PersistentArrayMap [meta cnt arr ^:mutable __hash]
  ...
  (-assoc [coll k v]
    (let [idx (array-map-index-of coll k)]
      (cond
        (== idx -1)
        (if (< cnt cljs.core.PersistentArrayMap/HASHTABLE_THRESHOLD)
...
(set! cljs.core.PersistentArrayMap/HASHTABLE_THRESHOLD 512)
  ...
  (-assoc! [tcoll key val]
    (if editable?
      (let [idx (array-map-index-of tcoll key)]
        (if (== idx -1)
          (if (<= (+ len 2) (* 2 cljs.core.PersistentArrayMap/HASHTABLE_THRESHOLD))
            (do (set! len (+ len 2))
                (.push arr key)
                (.push arr val)
                tcoll)
